...
| 容量上限 | **40GB**。再圧縮せず、4 日超と超過分を古い順に削除（下記） |
...
- 4 日を過ぎた画像を削除する（`Retention.retentionDays`）
- 4 日以内でも 40GB を超えたら、古い順に追加で削除する
...
容量の見積り: ピーク実績 9,383 枚/日 × 813KB = **7.3GB/日**。4 日で 29.1GB なので 40GB に収まる。
理論最大（24h 稼働・全フレーム差分あり）だと 13.4GB/日 で 4 日 53.6GB になるため、
...
│   ├── Retention.swift         # 4日保持 + 40GB 上限の削除（StorageBudget）
